        ...
        if (productExist) {
            ...
            try {
                if (productExist.totalprice && productExist.totalQuantity) {
                    const totPrice = parseInt(productExist.totalprice) + parseInt(parseInt(price) * parseInt(quantity));
                    const totQuantity = parseInt(productExist.totalQuantity) + parseInt(quantity);
                    await StockInfo.updateOne({ productname: productname }, { $set: { productname: productname, quantity: quantity, price: price, totalprice: totPrice, totalQuantity: totQuantity } })
...
const stocklist = async (req, res) => {
    try {
        const Stock = await StockInfo.find()
        res.send(Stock)
    }
    catch (err) {
        res.send({ msg: err })
